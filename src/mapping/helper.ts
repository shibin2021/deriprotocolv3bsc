...
import { expToBigDecimal, formatDecimal } from "../utils/converters"
import { AddMargin, PoolAbi } from "../../generated/Pool/PoolAbi"
import { ZERO_ADDRESS } from "../utils/constants"
import { VaultAbi } from "../../generated/Pool/VaultAbi"
import { ERC20Abi } from "../../generated/Pool/ERC20Abi"
import { VenusControllerAbi } from "../../generated/Pool/VenusControllerAbi"
import { VenusOracleAbi } from "../../generated/Pool/VenusOracleAbi"
import { VenusVTokenAbi } from "../../generated/Pool/VenusVTokenAbi"
...
export const initBTokens = (pool: Pool) : void => {
  ...
  vault.venusComptroller = vaultContract.comptroller()
  const venusControllerContract = VenusControllerAbi.bind(Address.fromBytes(vault.venusComptroller))
  ...
  for (let i = 0; i < allMarkets.length; i++) {
    ...
    log.info('- check market {}, asset {}, check {}',  [market.toHexString(), asset.toHexString(), poolContract.markets(Address.fromBytes(asset)).toHexString()])
    // ignore not support market
    if (
      asset == pool.tokenB0 ||
      asset == pool.tokenWETH ||
      poolContract.markets(Address.fromBytes(asset)) == market
    ) {
      ...
      bToken.bTokenPrice = formatDecimal(venusOracleContract.getUnderlyingPrice(Address.fromBytes(market)))
